...
def interactive_diarization(recognizer, similarity_dict, wav, wav_splits, x_crop=5, show_time=False):
    fig, ax = plt.subplots()
    ...
    def update(i):
        ...
        if similarity > 0.6:
            ...
            message2 = "similarity %s " % similarity
            color = _default_colors[best]
            # print(convert_voice_to_text(wav_splits[i], recognizer))
        # elif similarity > 0.65:
        #     message = "Speaker: %s (uncertain)" % name  
        #     message += "similarity %s " % similarity
